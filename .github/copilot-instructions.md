...
**no** `/specs` directory. Output is plain streamed text with minimal
ANSI styling.
...
- **Styling:** raw ANSI escape codes in `internal/tui/styles` (no external
  styling library).
...
internal/tui/styles/      ANSI color/style helpers + Ralph ASCII art splash.
